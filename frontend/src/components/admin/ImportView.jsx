...
const ProductEditModal = ({ product, onClose, onSave, authToken, onLogout }) => {
    ...
    const compress = (file) => new Promise((resolve, reject) => { const r = new FileReader(); r.onerror = () => reject(new Error('Không đọc được file ảnh')); r.onload = e => { const img = new Image(); img.onerror = () => reject(new Error('Định dạng ảnh không được hỗ trợ')); img.onload = () => { const c = document.createElement('canvas'); let w = img.width, h = img.height; const maxDim = 900; if (w > h && w > maxDim) { h = Math.round(h * maxDim / w); w = maxDim; } else if (h > maxDim) { w = Math.round(w * maxDim / h); h = maxDim; } c.width = w; c.height = h; c.getContext('2d').drawImage(img, 0, 0, w, h); let q = 0.8; let result = c.toDataURL('image/jpeg', q); while (result.length > 7 * 1024 * 1024 && q > 0.3) { q -= 0.1; result = c.toDataURL('image/jpeg', q); } resolve(result); }; img.src = e.target.result; }; r.readAsDataURL(file); });
                ...
                <div className="flex-1 overflow-y-auto p-5 space-y-4">
                    <div className="flex flex-col items-center gap-2">
                        <label className="w-24 h-24 bg-gray-50 rounded-2xl overflow-hidden border-2 border-dashed border-gray-200 flex items-center justify-center cursor-pointer hover:border-blue-400 transition-colors relative">
                            ...
                            <input type="file" className="absolute inset-0 opacity-0 cursor-pointer" accept="image/*" onChange={async e => { if (e.target.files[0]) { try { const c = await compress(e.target.files[0]); setForm(f => ({ ...f, image: c })); } catch (err) { alert('Không thể đọc ảnh: ' + err.message + '\nVui lòng dùng định dạng JPG, PNG hoặc WebP.'); } } }} />
                        </label>
